...
\textbf{Delete:} $O(\log n)$ amortized. Deletions are "lazy": the node is removed, but the tree is only rebuilt if the node count drops below $\frac{1}{2} \cdot \text{max\_nodes}$.
