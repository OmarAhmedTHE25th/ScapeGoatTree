...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"ScapeGoat Tree UI", WS_OVERLAPPEDWINDOW, 100, 100, 1600, 900, nullptr, nullptr, wc.hInstance, nullptr);
...
//----------------------------------------------------------------------------------------------------------------------
        ...
        ImGui::Separator();
        ImGui::Text("Status:");
        ImGui::TextColored(ImVec4(0,1,0,1), "%s", status);
        if (ImGui::Button("Check Balance/stats")) {
            output = tree.isBalanced();
        }



        if (ImGui::Button("In-Order Display")) {
            tree.displayInOrder();
            output = "In-Order Traversal:\n" + tree.getDisplayBuffer();
        }

        if (ImGui::Button("Pre-Order Display")) {
            tree.displayPreOrder();
            output = "Pre-Order Traversal:\n" + tree.getDisplayBuffer();
        }

        if (ImGui::Button("Post-Order Display")) {
...
//----------------------------------------------------------------------------------------------------------------------
        ...
        ImGui::PopStyleVar(3);
        ImGui::Render();
        constexpr float clear_color[4] = { 0.85f, 0.85f, 0.85f, 1.0f };
